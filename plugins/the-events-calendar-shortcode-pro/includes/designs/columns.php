...
function ecs_start_tag_columns( $output, $atts, $post ) {
    ...
    if ( ! doing_ecs_ajax_get_calendar_events() ) {
        ...
        $output .= '</style>';
        $output .= ecs_maybe_add_filterbar_output( $atts, $ecs_columns_design_count );
        $atts = ecs_maybe_alter_atts_for_filterbar( $atts );
        $output .= ecs_maybe_add_event_settings_script_output( $atts, $ecs_columns_design_count );
        $output .= '<div class="ecs-events ecs-clearfix ecs-grid ecs-grid-' . intval( $ecs_columns_design_count ) . ' columns-' . intval( $ecs_columns_design_count ) . '">';
    }
    $output .= '<div class="ecs-grid-sizer"></div><div class="ecs-gutter-sizer"></div>';
	return $output;
}
