        ...
        for (var key in tecsEvents) {
            if (tecsEvents.hasOwnProperty(key)) {
                $('#' + key).fullCalendar({
                    ...
                    header: {
                        left: tecEventCalendarSettings[key]['leftheader'],
                        center: tecEventCalendarSettings[key]['centerheader'],
                        right: tecEventCalendarSettings[key]['rightheader']
                    },
                    ...
                    // on mobile default to listMonth unless they specify a list___ view option
                    defaultView: $(window).width() < 514 ?
                        (
                            tecEventCalendarSettings[key]['defaultview'] && tecEventCalendarSettings[key]['defaultview'].substring(0, 4) === 'list' ?
                                tecEventCalendarSettings[key]['defaultview'] :
                                'listMonth'
                        ) : (
                            tecEventCalendarSettings[key]['defaultview'] ?
                                tecEventCalendarSettings[key]['defaultview'] :
                    ...
                    windowResize: function(view) {
                        if ($(window).width() < 514) {
                            ...
                            $(this).fullCalendar( 'changeView',
                                tecEventCalendarSettings[getTECSCalendarId(view)]['defaultview'] && tecEventCalendarSettings[getTECSCalendarId(view)]['defaultview'].substring(0, 4) === 'list' ?
                                tecEventCalendarSettings[getTECSCalendarId(view)]['defaultview'] :
                                'listMonth' );
                        } else {
