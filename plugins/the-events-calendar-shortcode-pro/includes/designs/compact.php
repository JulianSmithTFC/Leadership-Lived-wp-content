...
function ecs_start_tag_compact( $output, $atts, $post ) {
    ...
    if ( ! doing_ecs_ajax_get_calendar_events() ) {
        ...
        $output .= ecs_maybe_add_filterbar_output( $atts, $ecs_compact_design_count );
        $atts = ecs_maybe_alter_atts_for_filterbar( $atts );
        $output .= ecs_maybe_add_event_settings_script_output( $atts, $ecs_compact_design_count );

        $output .= '<div class="ecs-events compact compact-' . intval( $ecs_compact_design_count ) . '">';
    }
	return $output;
}
