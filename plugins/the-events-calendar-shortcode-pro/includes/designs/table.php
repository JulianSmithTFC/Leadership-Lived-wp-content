...
function ecs_start_tag_table( $output, $atts, $post ) {
    ...
    if ( ! doing_ecs_ajax_get_calendar_events() ) {
        ...
        $output .= '</style>';
        $output .= ecs_maybe_add_filterbar_output( $atts, $ecs_table_design_count );
        $atts = ecs_maybe_alter_atts_for_filterbar( $atts );
        $output .= ecs_maybe_add_event_settings_script_output( $atts, $ecs_table_design_count );
        $output .= '<div class="ecs-events ecs-clearfix ecs-table table-' . intval( $ecs_table_design_count ) . ' ecs-table-' . intval( $ecs_table_design_count ) . '">';
    }
    $output .= '<div class="ecs-table-row">';
	return $output;
}

function ecs_end_tag_table( $output, $atts, $post ) {
	$output = doing_ecs_ajax_get_calendar_events() ? '' : '</div>';
    $output .= '</div>';
    return $output;
}
