...
function ecs_start_tag_default( $output, $atts, $post ) {
    ...
    if ( ! doing_ecs_ajax_get_calendar_events() ) {
        // No need to replicate the same styles each request
        $output .= '<style>';
        ...
        $output .= ecs_maybe_add_filterbar_output( $atts, $ecs_default_design_count );
        $atts = ecs_maybe_alter_atts_for_filterbar( $atts );
        $output .= ecs_maybe_add_event_settings_script_output( $atts, $ecs_default_design_count );
        $output .= '<div class="ecs-events default default-' . intval( $ecs_default_design_count ) . '">';
    }
	return $output;
}
