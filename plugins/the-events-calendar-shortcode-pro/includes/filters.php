...
function ecs_exclude_specific_event_id( $args, $atts ) {
	if ( isset( $atts['exclude_id'] ) and trim( $atts['exclude_id'] ) ) {
		if ( 'current' == trim( $atts['exclude_id'] ) )
			$args['post__not_in'] = array( get_the_ID() );
		elseif ( trim( $atts['exclude_id'] ) )
			$args['post__not_in'] = array_map( 'intval', array_map( 'trim', explode( ',', $atts['exclude_id'] ) ) );
	}
	return $args;
}
...
add_filter( 'ecs_get_events_args', 'ecsp_date_range_filter', 8, 4 );
...
function ecsp_include_in_progress_events( $args, $atts ) {
	if ( isset( $atts['include_in_progress'] ) && Events_Calendar_Shortcode::isValid( $atts['include_in_progress'] ) &&
		...
		$events_in_range = $wpdb->get_results( $events_request );
        /**
         * We need to remove the from/to meta query, otherwise in-progress events
         * still won't show despite the post__in query.
         */
        $args['meta_query'] = array(
            'relation' => 'AND',
        );
		$args['post__in'] = wp_list_pluck( $events_in_range, 'ID' );
	}
	return $args;
}
// Set priority to 9 so it happens after the daterange meta query, but before any other meta query filters
add_filter( 'ecs_get_events_args', 'ecsp_include_in_progress_events', 9, 2 );
...
function ecsp_hide_recurring_after_first( $args, $atts ) {
	...
	if ( isset( $atts['hiderecurring'] ) and $events_calendar_shortcode::isValid( $atts['hiderecurring'] ) ) {
		...
		$args['hide_subsequent_recurrences'] = '1';

		// Replace the meta query with start_date as the meta_query doesn't work to show
        // the first recurring instance after the date anymore post-TEC 4.9
        if ( isset( $args['meta_query'], $atts['key'] ) and is_array( $args['meta_query'] ) ) {
            foreach ( $args['meta_query'] as $query_items_index => $query_items ) {
                if ( is_array( $query_items ) ) {
                    foreach ( $query_items as $index => $query_item ) {
                        if ( isset( $query_item['key'], $query_item['value'], $query_item['compare'] ) and $query_item['key'] === $atts['key'] ) {
                            $prefix = $atts['key'] === '_EventEndDate' ? 'ends_' : 'starts_';
                            if ( $query_item['compare'] === '>=' ) {
                                $args[ $prefix . 'after' ] = $query_item['value'];
                            } else {
                                $args[ $prefix . 'before' ] = $query_item['value'];
                            }
                            unset( $args['meta_query'][$query_items_index][$index] );
                            return $args;
                        }
                    }
                }
...
add_filter( 'ecs_get_events_args', 'ecsp_hide_recurring_after_first', 99, 2 );
