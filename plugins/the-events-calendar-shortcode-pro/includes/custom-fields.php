...
function ecsp_is_acf_installed() {
    return function_exists( 'acf_get_field_groups' ) and function_exists( 'acf_get_fields' );
}
...
function ecsp_add_acf_custom_fields( $atts ) {
    ...
    if ( !isset( $GLOBALS['ecsp_acf_fields'] ) or !is_array( $GLOBALS['ecsp_acf_fields'] ) ) {
        $GLOBALS['ecsp_acf_fields'] = array();
    }
    foreach ( (array) ecsp_get_acf_custom_fields() as $field ) {
        if ( !isset( $atts[ $field['name'] ]) ) {
            $GLOBALS['ecsp_acf_fields'][] = $field['name'];
            $atts[ $field['name'] ] = '';
        }
    }
    return $atts;
}
add_filter( 'ecs_shortcode_atts', 'ecsp_add_acf_custom_fields', 999 );

function ecsp_filter_by_acf_custom_fields( $args, $atts, $meta_date_date, $meta_date_compare ) {
    if ( isset( $GLOBALS['ecsp_acf_fields'] ) and is_array( $GLOBALS['ecsp_acf_fields'] ) ) {
        foreach ( $GLOBALS['ecsp_acf_fields'] as $field_name ) {
