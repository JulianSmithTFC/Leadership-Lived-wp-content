...
// Use the same markup as the columns design within each table cell
?>
			...
			<div class="summary">
				...
				<?php if ( in_array( 'excerpt', $contentorder ) and Events_Calendar_Shortcode::isValid( $atts['excerpt'] ) ): ?>
                    <div class="ecs-excerpt">
                        ...
                        <?php echo apply_filters( 'ecs_event_excerpt', Events_Calendar_Shortcode::get_excerpt( $excerptLength ), $atts, $post, $excerptLength ) ?>
                    </div>
				<?php endif; ?>
...
// Decide whether to start a new table row or not based on the number of columns
$columns = intval( $atts['columns'] ) > 0 && intval( $atts['columns'] ) <= 6 ? intval( $atts['columns'] ) : 3;
if ( ! isset($GLOBALS['ecs-table-cell-count-' . $ecs_table_design_count]) )
	$GLOBALS['ecs-table-cell-count-' . $ecs_table_design_count] = 0;
$GLOBALS['ecs-table-cell-count-' . $ecs_table_design_count]++;
?>
<?php if ( $GLOBALS['ecs-table-cell-count-' . $ecs_table_design_count] % $columns === 0 ): ?>
	</div>
	<div class="ecs-table-row">
<?php endif; ?>